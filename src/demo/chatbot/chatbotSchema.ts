...
// re-import. `embed` is the cross-schema hook: a Message can link to any
// other node (a Form, a Story scene, anything), and the merged registry takes
// care of rendering — chatbot doesn't need to know the embedded node's type.
// The text `content` literal stays as-is; `embed` is an additional, optional
// link that augments the bubble.
...
export type ChatbotGraphDocument = {
  context: typeof chatbotContext;
  nodes: { [id: string]: ChatbotGraphNode };
};
