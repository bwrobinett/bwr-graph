...
- `graphViewSchema.ts` — Zod schema for the canonical `GraphView` root node plus the `graphViewContext` (no link properties of its own).
...
- `graphViewExampleGraph.ts` — JSON-like plain `GraphDocument` used by the top-level demo seed.
